...
관리자 페이지에서 Edge Function base URL, Supabase anon/publishable key, `ADMIN_API_KEY`를 입력하면 `list-candidates`로 후보를 불러오고, 후보별 카테고리/상태/요약을 조정한 뒤 `promote-candidate`로 `published=false` 초안을 만들 수 있습니다. 이후 `list-trends`로 승격된 트렌드를 확인하고 `update-trend`로 내용을 수정한 뒤 `set-trend-published`로 공개/비공개 상태를 전환할 수 있습니다. `ADMIN_API_KEY`는 코드에 저장하지 않고 현재 브라우저 세션에만 보관합니다. Edge Function을 사용할 수 없는 경우를 대비해 JSON 붙여넣기와 승격 SQL 복사 기능도 유지합니다.
...
- `update-trend`: 승격된 트렌드의 순위, 키워드, 카테고리, 상태, 태그, 요약을 수정합니다.
...
supabase functions deploy update-trend
```
...
- `supabase/functions-dashboard/update-trend.ts`: `update-trend` 함수 에디터에 전체 복사/붙여넣기
